...
def toon_bord(board):
    ...
    # ===== 2. Add ROW LABELS on the left of each row =====
    for r in range(rows):
        ax = axes[r, 0]
        ax.set_ylabel(
            row_labels[r],
            rotation=0,
            labelpad=20,  # pushes label leftward
            va="center",
            ha="center",
        )

    # ===== 3. Add COLUMN LABELS centered under each column =====
    for c in range(cols):
        ax = axes[rows - 1, c]
        ax.set_xlabel(
            col_labels[c],
            labelpad=20,  # pushes label downward
            ha="center",
            va="center",
        )

    # ===== 4. Build roster gridlines using spines =====
...
def geldige_zetten_paard(bord, kleur, van_rij, van_kol):
    ...
    for zet in zetten:
        if (zet[0] >= 0) and (zet[0] <= 7) and (zet[1] >= 0) and (zet[1] <= 7) and bord[zet[0]][zet[1]][0] != kleur:
...
def geldige_zetten_koningin(bord, kleur, van_rij, van_kol):
    ...
    geldige_zetten = geldige_zetten_loper(bord, kleur, van_rij, van_kol) + geldige_zetten_toren(bord, kleur, van_rij, van_kol)

    return geldige_zetten


def doe_zet_met_controle(bord, kleur, van, naar):
...
def doe_zet_zonder_controle(bord, kleur, van, naar):

    volgend_bord = [rij[:] for rij in bord]

    volgend_bord[naar[0]][naar[1]] = bord[van[0]][van[1]]
    volgend_bord[van[0]][van[1]] = ['leeg', 0]
    ...
    for i in range(8):
        for j in range(8):
            if bord[i][j][0] == kleur:
                ...
                for zet in zetten:
                    bord_v = doe_zet_zonder_controle(bord, kleur, [i, j], zet)
...
def checkmate(bord, kleur):
    ...
    if not bedreigingen:
        ...
        for z in zetten:
            bord_v = doe_zet_zonder_controle(bord, kleur, z[0], z[1])
...
def alle_geldige_zetten_score(bord, kleur):
    ...
    for zet in geldige_zetten:
        ...
        bord_v = doe_zet_zonder_controle(bord, kleur, zet[0], zet[1])
        ...
        if check_v_other:
            ...
            if ch_mt:
                geldige_zetten_score.append([zet[0], zet[1], 6]) # schaakmat
                break
            else:
                geldige_zetten_score.append([zet[0], zet[1], 2]) # schaak
...
def N_games(player_1, player_2, N, max_zetten):
    ...
    for i in range(N):
        ...
        if i % 2 == 0:
            ...
            bord, wit, zwart = spel(speler_wit, speler_zwart, max_zetten, verbose=False, toon=False)
    ...
    def make_move(self, bord):
        ...
        if ch:
            ...
            if ch_mt == True:
                ...
                bord = doe_zet_zonder_controle(bord, self.kleur, zet[0], zet[1]) 
                return bord, self.status
            ...
            if zetten_score:
                ...
                if m == 6:
                    ...
                    for zet in zetten_score:
                        bord_v = doe_zet_zonder_controle(bord, self.kleur, zet[0], zet[1])
